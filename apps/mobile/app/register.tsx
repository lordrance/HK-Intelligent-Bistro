      ...
      if (!res.ok) {
        const msg =
          ...
          typeof (body as { error: unknown }).error === "string"
            ...
            : `HTTP ${res.status}`;
        setError(msg === "invalid_body" ? "Check email and password (min 8 characters)." : `Could not register (${msg}).`);
        return;
      }
      if (
        !body ||
    ...
    <KeyboardAvoidingView className="flex-1 bg-bistro-bg" behavior={Platform.OS === "ios" ? "padding" : undefined}>
      <View className="flex-1 bg-bistro-bg">
        ...
        >
          ...
          <Text className="mb-7 text-sm leading-[22px] text-bistro-muted">
            Your password is stored securely on the server. You need an account to browse the menu.
          </Text>
          ...
          <View className="mb-4">
            ...
            <Input placeholder="At least 8 characters" secureTextEntry value={password} onChangeText={setPassword} />
          </View>

          {error ? <Text className="mb-3 text-sm text-red-300">{error}</Text> : null}

          <Button className="mt-2" variant="primary" disabled={busy} onPress={() => void onSubmit()}>
            {busy ? <ActivityIndicator color={T.bg} /> : <ButtonText variant="primary">Register</ButtonText>}
          </Button>

          <View className="mt-4 flex-row flex-wrap gap-2">
            <Text className="text-bistro-muted">Already have an account?</Text>
            <Link href="/login" asChild>
              <Pressable>
                <Text className="font-extrabold text-bistro-gold-soft">Sign in</Text>
              </Pressable>
            </Link>
          </View>
        </ScrollView>
